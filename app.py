...
def handle_join(event, client, logger):
    ...
    if new_user == JAME_ID and _jame_self_invite_pending:
        logger.info("Jame rejoined via /lopio invite — skipping kick logic.")
        _jame_self_invite_pending = False
        return
            ...
            if not human_members:
                ...
                try:
                    ...
                    if "already_in_channel" in str(e):
                        logger.info("Jame is in channel, stop panicking.")
                    else:
...
def handle_lopio(ack, command, client, respond, logger):
    ...
    if subCmd == "log":
        ...
        respond(
            ":jame-hehe: Not a valid command! Try one of these:\n"
